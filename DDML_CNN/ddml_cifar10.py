...
class DDMLNet(nn.Module):
    def __init__(self, device, beta=1.0, tao=5.0, b=1.0, learning_rate=0.001):
        ...
        # [batch_size, 60, 8, 8]
        self.fc1 = nn.Linear(self.cnn_output_dim, self.cnn_output_dim // 2)
        self.fc2 = nn.Linear(self.cnn_output_dim // 2, self.cnn_output_dim // 4)
        self.fc3 = nn.Linear(self.cnn_output_dim // 4, self.cnn_output_dim // 8)
        self.fc4 = nn.Linear(self.cnn_output_dim // 8, 10)
